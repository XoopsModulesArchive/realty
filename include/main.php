<?php
	...
	//handles everything to do with users logging in
	{
		...
		if ($user_name == "" AND $username == "")
		{
			...
			echo "<form action=\"".$config[baseurl]."/admin/emailpass.php\"><p>$lang[enter_your_email_address_for_pass]<BR><input type=text name=email></p><p><input type=submit></p></form>";xoops_cp_footer();
				exit;
		}
		elseif ($user_name != "" OR $username != "")
		{
			...
			elseif ($num == 0)
			{
				...
				echo "<h3><img src=\"../images/warning.gif\">&nbsp;&nbsp;$lang[login_failed]</h3>";
				...
				echo "<form action=\"emailpass.php\"><p>$lang[enter_your_email_address_for_pass]<BR><input type=text name=email></p><p><input type=submit></form></p>";xoops_cp_footer();
				exit;
			} // end elseif
		} // end elseif
	...
	if ($user_name == "" AND $username == "")
		{
		echo "<p><br><b><img src=\"../images/warning.gif\">$lang[visitor_alert_phrase]</b></P>";
		echo "<p><form action=\"$PHP_SELF\" method=\"post\">$lang[admin_challenge_phrase]</P>";
		echo "<p>$lang[admin_login_name]: <input type=\"text\" name=\"user_name\"></p> ";
		echo "<p>$lang[admin_password]: <input type=\"password\" name=\"user_pass\"></p><p><input type=\"submit\" value=\"Log In\"></form></p>";
		echo "<form action=\"".$config[baseurl]."/admin/emailpass.php\"><p>$lang[enter_your_email_address_for_pass]<BR><input type=text name=email></p><p><input type=submit></form></p>";CloseTable();
		exit;
		} 
	elseif ($user_name != "" OR $username != "")
		{
...
if ($type == "vTour")
				...
				if ($type == "autovTour")
				{
					...
					$check_size="";
					// move the file so we can check the width
					move_uploaded_file($HTTP_POST_FILES['userfile']['tmp_name'],"$config[autovTour_upload_path]/$save_name");
					$imagesize = filesize("$config[autovTour_upload_path]/$save_name");
					if ($imagesize == "" || $imagesize > $config[max_autovTour_upload_size])
					{
						$check_size="$lang[upload_too_large].";
						if(!unlink("$config[autovTour_upload_path]/$save_name"))
						{
							DIE ("Can't delete the file!");
						}
						DIE ("$check_size");
					...
					// check width
					$check_width="";
					$imagedata = GetImageSize("$config[autovTour_upload_path]/$save_name");
					$imagewidth = $imagedata[0];
					$imageheight = $imagedata[1];
					if ($imagewidth == "" || $imagewidth < 2 || $imagewidth > $config[max_autovTour_upload_width])
					{
						$check_width = "$lang[upload_too_wide].";
						if (!unlink("$config[autovTour_upload_path]/$save_name"))
						{
					...
					if ($check_width == "")
					{
						...
						$thumb_name = $save_name; // by default -- no difference... unless...
						$caption = make_db_safe($caption);
						$sql = "INSERT INTO " . $config[table_prefix] . "autovTourImages (auto_id, user_id, file_name, thumb_file_name) VALUES ('$edit', '$owner', '$save_name', '$thumb_name')";
						$recordSet = $conn->Execute($sql);
						if ($recordSet === false)
						{
							log_error($sql);
						}
						log_action ("$lang[log_uploaded_auto_vtour] $save_name");
						chmod("$config[autovTour_upload_path]/$save_name",0777);
					} // end if ($check_width != "")
				} // end if $type == "autovTour"
			...
			else
			{
				...
				echo "<p><b><img src=\"../images/warning.gif\">$lang[upload_failed]</b> $pass_the_upload</p>";xoops_cp_footer();
			}
		} // end if
		else
		{
	...
	echo " <!-- powered by Real Estate Listing Engine by http://www.liquidgfx.com -->";
