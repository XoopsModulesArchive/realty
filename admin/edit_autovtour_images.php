...
global $action, $id, $cur_page, $lang, $conn, $config, $edit, $pic, $submit;
	...
	// does this person have access to these auto?
	if ($admin_privs != "yes")
		{
	...
	if ($action == "update_pic")
		...
		if ($admin_privs == "yes")
			{
			$sql = "UPDATE " . $config[table_prefix] . "autovTourImages SET caption = $sql_caption, description = $sql_description, rank = $sql_rank WHERE ((auto_id = $sql_edit) AND (file_name = $sql_pic))";
			}
		else
			{
			$sql = "UPDATE " . $config[table_prefix] . "autovTourImages SET caption = $sql_caption, description = $sql_description, rank = $sql_rank WHERE ((auto_id = $sql_edit) AND (file_name = $sql_pic) AND (user_id = $userID))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
		echo "<p>$lang[vTour] '$pic' $lang[has_been_updated]</p>";
		log_action ("$lang[log_updated_auto_vTour] $pic");
		}
	...
	if ($action == "delete_pic")
		...
		if ($admin_privs == "yes")
			{
			$sql = "SELECT file_name, thumb_file_name FROM " . $config[table_prefix] . "autovTourImages WHERE ((auto_id = $sql_edit) AND (ID = $pic_id))";
			}
		else
			{
			$sql = "SELECT file_name, thumb_file_name FROM " . $config[table_prefix] . "autovTourImages WHERE ((auto_id = $edit) AND (ID = $pic_id) AND (user_id = $userID))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
		...
		if ($admin_privs == "yes")
			{
			$sql = "DELETE FROM " . $config[table_prefix] . "autovTourImages WHERE ((auto_id = $sql_edit) AND (file_name = '$file_name'))";
			}
		else
			{
			$sql = "DELETE FROM " . $config[table_prefix] . "autovTourImages WHERE ((auto_id = $sql_edit) AND (file_name = '$file_name') AND (user_id = '$userID'))";
			}
		$recordSet = $conn->Execute($sql);
			if ($recordSet === false) log_error($sql);
		...
		if (!unlink("$config[autovTour_upload_path]/$file_name")) die("$lang[alert_site_admin]");
		if ($file_name != $thumb_file_name)
			{
			if (!unlink("$config[autovTour_upload_path]/$thumb_file_name")) die("$lang[alert_site_admin]");
			}
		log_action ("$lang[log_deleted_auto_vTour] $file_name");
		echo "<p>$lang[autovTour] '$file_name' $lang[has_been_deleted]</p>";
	...
	if ($action == "upload")
		...
		if ($admin_privs == "yes")
			...
			handleUpload("autovTour",$edit, $owner);
			}
		else
			{
			handleUpload("autovTour",$edit, $userID);
			}
	...
	<tr><td colspan="2" class="row_main"><h3><?php echo $lang[edit_autovTour]?> -- 
	<?php
	if ($admin_privs == "yes")
		{
	...
	if ($admin_privs == "yes")
		{
		$sql = "SELECT ID, caption, file_name, thumb_file_name, description, rank FROM " . $config[table_prefix] . "autovTourImages WHERE (auto_id = '$edit') ORDER BY rank";
		}
	else
		{
		$sql = "SELECT ID, caption, file_name, thumb_file_name, description, rank FROM " . $config[table_prefix] . "autovTourImages WHERE ((auto_id = '$edit') AND (user_id = '$userID')) ORDER BY rank";
		}
	$recordSet = $conn->Execute($sql);
	if ($recordSet === false) log_error($sql);
	...
	while (!$recordSet->EOF)
		...
		$imagedata = GetImageSize("$config[autovTour_upload_path]/$file_name");
		$imagewidth = $imagedata[0];
		$imageheight = $imagedata[1];
		$shrinkage = $config[thumbnail_width]/$imagewidth;
		$displaywidth = $imagewidth * $shrinkage;
		$displayheight = $imageheight * $shrinkage;
		$filesize = filesize("$config[autovTour_upload_path]/$file_name");
		$filesize = $filesize/1000; // to get k
		
		// now grab the thumbnail data
		$thumb_imagedata = GetImageSize("$config[autovTour_upload_path]/$thumb_file_name");
		$thumb_imagewidth = $thumb_imagedata[0];
		$thumb_imageheight = $thumb_imagedata[1];
		$thumb_filesize = filesize("$config[autovTour_upload_path]/$thumb_file_name");
		...
		echo "<img src=\"$config[auto_view_vTour_path]/$thumb_file_name\" height=\"$displayheight\" width=\"$displaywidth\" border=\"1\"> ";
		...
		echo "</td><td align=\"center\" class=\"image_row_$count\"><img src=\"$config[auto_view_vTour_path]/$file_name\" border=\"1\">";
	...
	echo "</table>";
	if ($num_images < $config[max_autovTour_uploads])
		{
	?>
		...
		<table border=0 cellspacing=0 cellpadding=0>
			<tr>
				<td colspan="2">
					<h3><?php echo $lang[upload_a_autovTour] ?></h3>
				</td>
			...
			<tr>
				...
				<td>
					...
					<input type="hidden" name="MAX_FILE_SIZE" value="<?php echo $max_autovTour_upload_size ?>">
...
?>
	<?php
		} // end if $num_images <= $config[max_autovTourImages_uploads]
	?>
